...
# from mlx_api import __version__
# from mlx_api.models.utils import model_info, flags, recommended_usage
# from mlx_api.models.rag.utils import get_prompt, lang_dict
...
#         self,
#         *,
#         # colors.Color(c50="#F7FBFF",c100="#EFF6FF",c200="#D7E9FE",c300="#BFDBFD",c400="#90C0FC",c500="#60A5FA",c600="#5695E1",c700="#3A6396",c800="#2B4A71",c900="#1D324B",c950="#1A2D44",),
#         primary_hue: colors.Color | str = "#7c3aed",
#         secondary_hue: colors.Color | str = "#0d9488",
#         neutral_hue: colors.Color | str = "#f3f4f6",
...
#         gr.Info(
#             f'Model {model_name} is downloading. Please check terminal to see the progress.')
#         snapshot_download(
#             repo_id=mlx_config[model_name], local_dir=local_model_dir)
...
#                             "eos_token": tokenizer_config_['eos_token']}
#     try:
...
#     if emb is None:
#         gr.Info(
#             "Loading embedding model (first time may take longer, check terminal to check download progress)")
...
#     else:
#         response = stream_generate(model, tokenizer, prompt, temp=temp,
#                                    max_tokens=max_tokens, repetition_penalty=repetition_penalty, top_p=top_p,)
...
#         model_configs_name = os.path.join(
#             local_config_dir, f"{model_name}-{quantization}.yaml")
#     else:
#         new_model_dict = {
#             'original_repo': original_repo,
#             'mlx-repo': mlx_repo,
...
#         model_configs_name = os.path.join(
#             local_config_dir, f"{model_name}.yaml")
...
#     with gr.Tab("Chat"):
#         with gr.Row():
#             system_prompt = gr.Textbox(placeholder="System prompt (blank will use model's default system prompt)",
#                                        label="System Prompt", interactive=True, render=False, scale=9)
#             with gr.Column(scale=1):
#                 with gr.Row():
#                     config_text = gr.Markdown(
#                         '### Configuration', show_label=False)
...
#                     "Model Not Loaded", label="Model Status")
...
#                         upload, inputs=upload_button, outputs=[
#                             url, index_status]
#                     )
...
#                     default_prompt_btn = gr.Button(
#                         "Default Prompt", variant="secondary", scale=1)
#                     default_prompt_btn.click(
#                         reset_sys_prompt, outputs=[system_prompt])

#                 gr.ChatInterface(
#                     chatbot=gr.Chatbot(
#                         height=562, render=False, layout="bubble", bubble_full_width=False),
...
#                     undo_btn='Undo',
#                     clear_btn='Clear',
#                     additional_inputs=[temp_slider, max_gen_token, rep_penalty,
#                                        retrieve_docs, top_p, system_prompt, model_name],
#                 )
...
#             playground = gr.Textbox(placeholder="Enter your text...", interactive=True,
#                                     lines=31, scale=4, show_label=False, render=False)
#             total_token_completion = gr.Markdown(
#                 "Input Tokens: 0, Output Tokens: 0, Total Tokens: 0", label="Total Token", show_label=False, render=False)
#             with gr.Column(scale=1):
#                 with gr.Row():
#                     config_text = gr.Markdown(
#                         '### Configuration', show_label=False)
...
#                 submit_completion.click(completions, inputs=[playground, temp_slider_completion, max_gen_token_completion, rep_penalty_completion,
#                                         top_p_completion, stop_sequence_completion, model_name_com], outputs=[playground, total_token_completion])
...
#                 available_models = gr.DataFrame(headers=[
#                                                 'Available Models'], value=available_list, scale=3, show_label=False, height=500)
#             with gr.Column(scale=2):
#                 support_lang_list = sorted(SUPPORTED_LANG)[:-1]
#                 support_lang_list.append('Multilingual')
#                 gr.Markdown('## Add new model')
#                 new_model_repo = gr.Textbox(info='Original Repo', show_label=False,
#                                             placeholder="Original Repo (i.e teknium/OpenHermes-2.5)", interactive=True)
#                 new_mlx_model_repo = gr.Textbox(
#                     info='MLX Repo', show_label=False, placeholder="MLX Repo (i.e mlx-community/OpenHermes-2.5-Mistral-7B)", interactive=True)
#                 with gr.Row():
#                     quantization_mode = gr.Dropdown(label='Quantization Mode', choices=[
#                                                     'none', '4bit', '8bit'], interactive=True, value='none')
#                     language_choices = gr.Dropdown(
#                         label="Default Language", choices=support_lang_list, value="English", interactive=True)
#                 default_sys_prompt = gr.Textbox(
#                     info="Default System Prompt", show_label=False, placeholder="ex: You are a helpful assistant.", interactive=True)
#                 with gr.Row():
#                     add_model_button = gr.Button(
#                         "Add Model", variant="primary", scale=5)
#                     quit_button = gr.Button(
#                         "Quit", variant='stop', scale=2, elem_classes='red-btn')

#                 add_model_status = gr.Markdown("Status: None")
#                 add_model_button.click(add_model, inputs=[new_model_repo, new_mlx_model_repo, quantization_mode,
#                                        language_choices, default_sys_prompt, available_list_state], outputs=[add_model_status, available_models])
#                 quit_button.click(terminate_process,
#                                   outputs=[add_model_status])
#         with gr.Row():
#             with gr.Column(scale=3):
